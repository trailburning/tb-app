...
$include_path = get_include_path().PATH_SEPARATOR.$api_root.'lib/'.PATH_SEPARATOR.$api_root.'3rdparty/';
...
$app->get('/route/import/gpx', function () {
	echo '<form action="/route/import/gpx" method="post" enctype="multipart/form-data"><input type="file" name="gpxfile"><input type="submit"></form>';
	return;
});

$app->post('/route/import/gpx', function () {
  	...
  	if (array_key_exists("gpxfile", $_FILES)) {
  		if ($_FILES['gpxfile']['error'] == 0) {
  			...
  			try {
				...
				try {
					$db = new TB\Postgis($db_config['driver'].':host='.$db_config['host'].';dbname='.$db_config['db'], $db_config['user'], $db_config['password'], array(PDO::ATTR_PERSISTENT => true));
				}
...
$app->get('/route/:id', function ($eventid) {
	require_once 'importers/GPX.php';
	require_once 'databases/postgis.php';
